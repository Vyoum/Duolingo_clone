...
export function LiveRewards({ profile = false }: { profile?: boolean }) {
  ...
  return <section className="live-panel">
    {profile ? <><h1>{data.name}&apos;s progress</h1><p className="subtitle">Every lesson counts. Here’s how far you’ve come.</p><div className="learner-stats"><div><strong>🔥 {data.streak}</strong><span>Day streak</span></div><div><strong>⚡ {data.xp}</strong><span>Total XP</span></div><div><strong>♥ {data.hearts}</strong><span>Hearts available</span></div><div><strong>{data.lessons_completed}</strong><span>Lessons completed</span></div></div>{data.next_heart_at && <p className="subtitle">Next heart at {new Date(data.next_heart_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>}</> : <><h1>Daily quests</h1><p className="subtitle">Little challenges. Lasting progress. Resets at midnight UTC.</p><Goals items={data.quests} /></>}
    <h2>Achievements</h2><Goals items={data.achievements} />
  </section>;
}
